...
class Make_Paper_Cmd_Cog(commands.Cog):
    ...
    @commands.command()
    async def race_close_timer(self, ctx):
        
        # json dataの取得
        race_json, _ = race_get.race_json_data()
        json_keys_list = list(race_json.keys())
        
        race_start_time_list = []
        race_info_tuple_list = []
        for json_key in json_keys_list:
            # 各データの取得
            race_start_time = race_json[json_key]['race_times']
            race_name = race_json[json_key]['race_name']
            races_info_tuple = (json_key, race_name)
            
            race_start_time_list.append(race_start_time)
            race_info_tuple_list.append(races_info_tuple)

        # 出走時刻のみへの変換
        start_time_list = [ x[:5] for x in race_start_time_list ]
        # 出走時刻->key、(レースID, レース名)->value
        race_start_time_dict = dict(zip(start_time_list, race_info_tuple_list))
        race_start_dict = race_start_time_dict

        print('======== <== レース名及び出走時刻dict ==> ========')
        print(race_start_dict)
        print('='*50)

        self.end_vote.start(ctx, race_start_dict, start_time_list)
        await ctx.channel.send('@here 自動締め切りタイマーを起動しました。\n予想の投票締め切り時間は、各レースの出走時間【15分前】になります。\nお早めに投票お願い致します。')

        # n分前計算の告知embed
        start_time_embed = discord.Embed(title="各レースの投票に関する時刻", color=0x00ff00, description="予想投票が可能レースの各時刻に関するお知らせです")
        for start_key in start_time_list:   # 各レースごとに計算
            # 時間の分割
            sep_times = start_key.split(':')
            start_hour = sep_times[0]
            start_min = sep_times[1]

            # 15分前処理
            before_15min = int(start_min)-15
            if before_15min < 0:
                before_15min = str(60+before_15min).zfill(2)
                start_15min_hour = str(int(start_hour)-1).zfill(2)
            else:
                before_15min = str(before_15min).zfill(2)
                start_15min_hour = str(start_hour).zfill(2)
            
            before_15min_time = start_15min_hour + ':' + before_15min
            
            # 20分前処理
            before_20min = int(start_min)-20
            if before_20min < 0:
                before_20min = str(60+before_20min).zfill(2)
                start_20min_hour = str(int(start_hour)-1).zfill(2)
            else:
                before_20min = str(before_20min).zfill(2)
                start_20min_hour = str(start_hour).zfill(2)

            before_20min_time = start_20min_hour + ':' + before_20min


            # レース情報の取得
            race_infos_tuple = race_start_time_dict[start_key]
            start_race_id = race_infos_tuple[0]     # レースID
            start_race_name = race_infos_tuple[1]   # レース名
            start_time = start_key                  # レース出走時刻
            start_15min_time = before_15min_time    # 出走時刻15分前
            start_20min_time = before_20min_time    # 出走時刻20分前

            # Embedの生成
            start_time_embed.add_field(name=f'【{start_race_name}】',value=f'レースID : {start_race_id}',inline=False)
            start_time_embed.add_field(name='レース出走時刻',value=f'{start_time}',inline=True)
            start_time_embed.add_field(name='bot内予想投票締切時刻',value=f'{start_15min_time}',inline=True)
            start_time_embed.add_field(name='bot締切5分前',value=f'{start_20min_time}',inline=True)


        # Embedの送信
        race_start_time_Embed_msg = await ctx.channel.send(embed=start_time_embed)

        print('予想投票自動締め切りタイマーの起動')
        print('1分ごとに現在時刻を確認し、5分前に投票を自動で締め切ります')

    
    @tasks.loop(seconds=60)
    #async def end_vote(self, ctx, *input_race_id):self, ctx, *input_race_id):
    async def end_vote(self, ctx, race_start_dict, start_time_list):
        
        # 現在時刻の取得
        dt_now = datetime.datetime.now()
        dt_15min = dt_now + datetime.timedelta(minutes=15)
        dt_20min = dt_now + datetime.timedelta(minutes=20)

        today_data = str(dt_now.year) + str(dt_now.month).zfill(2) + str(dt_now.day).zfill(2)

        now_time = str(dt_now.hour).zfill(2) + ':' + str(dt_now.minute).zfill(2)    # 現在時刻        
        after_15min_time = str(dt_15min.hour).zfill(2) + ':' + str(dt_15min.minute).zfill(2)   # 現在時刻より15分後
        after_20min_time = str(dt_20min.hour).zfill(2) + ':' + str(dt_20min.minute).zfill(2)   # 現在時刻より20分後

        print(f'現在時刻 : {now_time}, 15分後時刻 : {after_15min_time}, 20分後時刻 : {after_20min_time}')

        # 投票締め切り告知(20分前)
        if after_20min_time in start_time_list:
            # 該当レースの取得
            temp_target_race_name = race_start_dict[after_20min_time]
            target_race_name = temp_target_race_name[1]
            await ctx.channel.send(f'@here 【{target_race_name}】の予想投票締め切り『5分前』です。\n予想投票はお早めにお願い致します。')
            print(f'【{target_race_name}】締め切り5分前告知')

        # 投票締め切り通知15分前
        if after_15min_time in start_time_list:
            temp_target_race_name = race_start_dict[after_15min_time]
            target_race_name = temp_target_race_name[1]
            race_id = temp_target_race_name[0]
            await ctx.channel.send(f'@here 【{target_race_name}】の予想投票を締め切ります')
            print('*'*40)
            print(f'【{target_race_name}】を締め切りました。新聞を生成します')
            print('*'*40)
...
def setup(bot):
    bot.add_cog(Make_Paper_Cmd_Cog(bot))
